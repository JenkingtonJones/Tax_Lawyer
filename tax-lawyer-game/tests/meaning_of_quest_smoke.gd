...
func _run() -> void:
	...
	_check(map_scene.get_node("WorldBlockers").get_child_count() == 11, "City map should have 11 solid world blockers")
...
func _assert_map_locations_reachable(map_scene: Node2D) -> void:
	const GRID_STEP := 8
	const GRID_ORIGIN := Vector2(50, 92)
	const GRID_SIZE := Vector2i(148, 65)
	var goals := {
	...
	while cursor < queue.size() and reached.size() < goals.size():
		...
		for direction in directions:
			...
			query.transform = Transform2D(0.0, next_point + Vector2(0, 20))
