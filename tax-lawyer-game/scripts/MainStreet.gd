...
const DIALOGUE_BOX_MENU_POSITION := Vector2(305, 118)
const DIALOGUE_BOX_RESULT_POSITION := Vector2(522, 118)
const DIALOGUE_BOX_MENU_SCALE := Vector2(0.68, 0.54)
const DIALOGUE_BOX_RESULT_SCALE := Vector2(1.30, 0.54)
const DIALOGUE_TEXT_MENU_RECT := Rect2(56, 56, 480, 132)
const DIALOGUE_TEXT_RESULT_RECT := Rect2(56, 56, 928, 132)
const CHOICE_BOX_MENU_POSITION := Vector2(812, 118)
...
const FACILITATOR_FOLLOW_UPS := [
	"She takes a breath and explains that several receipts are chewed, two folders are missing, and one invoice may now be confetti.",
	"She remembers a second box in the hall closet. It may contain the payroll slips, or it may contain old holiday cards. She is not optimistic.",
	"That gives you enough to stop the sidewalk intake. This needs a proper appointment and a desk that is not outdoors.",
]
...
func _setup_text_layout() -> void:
	dialogue_text.custom_minimum_size = DIALOGUE_TEXT_MENU_RECT.size
	...
	_set_choice_texts(INITIAL_CHOICE_TEXTS)
...
func _select_choice(option: int) -> void:
	...
	if missing_docs_active:
		_select_facilitator_response()
		return

	match option:
		1:
			_start_missing_docs_story()
		2:
			_queue_map_return(true)
...
func _start_missing_docs_story() -> void:
	...
	facilitator_count = 0
	_set_menu_layout()
	_set_dialogue_text(STORY_INIT_DIALOGUE)
	_set_choice_texts(FACILITATOR_CHOICE_TEXTS)
	choice_1.grab_focus()

func _select_facilitator_response() -> void:
	var follow_up: String = FACILITATOR_FOLLOW_UPS[facilitator_count]
	facilitator_count += 1
	if facilitator_count >= FACILITATOR_FOLLOW_UPS.size():
		missing_docs_active = false
		_queue_map_return(true)
		_apply_choice(-10, -10, 0, 0, "%s\n\n%s" % [follow_up, APPOINTMENT_NEEDED])
	else:
...
func _set_choice_texts(choice_texts: Array) -> void:
	choice_1.text = choice_texts[0]
	choice_2.text = choice_texts[1]
	choice_3.text = choice_texts[2]
...
func _queue_map_return(unlock_office: bool) -> void:
	var game_state := _game_state()
	if unlock_office:
		game_state.set("office_unlocked", true)

	game_state.set("map_spawn", "tax_office")
	pending_scene_path = HOME_GRID_SCENE_PATH
...
func _load_state() -> void:
	var game_state := _game_state()
	money = int(game_state.get("money"))
	stamina = int(game_state.get("stamina"))
